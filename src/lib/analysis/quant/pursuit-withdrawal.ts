...
export function detectPursuitWithdrawal(
  ...
  const WITHDRAWAL_THRESHOLD_MS = 2 * 60 * 60 * 1000; // 2 hours of silence
  const MIN_CONSECUTIVE = 3; // minimum messages to count as pursuit
  ...
  const pursuer = sorted[0]?.[0] ?? participantNames[0];
  const withdrawer = sorted.length > 1 ? sorted[sorted.length - 1][0] : participantNames[1];
